...
class PublicHooks
{
    ...
    public function validateAddressCheckout() {
        foreach (Constants::CHAIN_CODES as $chain) {
            if (isset($_POST['recipient_blockchain_address_' . $chain])) {
                $recipient_address = sanitize_text_field($_POST['recipient_blockchain_address_' . $chain]);
                if (!AddressValidator::isETHAddress($recipient_address)) {
    ...
    public function checkForPremiumPlatinum($order_id) {
        $order = wc_get_order($order_id);
        foreach ($order->get_items() as $item) {
            $product = get_post( $item->get_product_id() );
            $slug = $product->post_name;
            if (in_array($slug, array_values(CONSTANTS::SUBSCRIPTION['SLUG']))) {
                $plan = CONSTANTS::SUBSCRIPTION['SLUG']['PREMIUM'] === $slug ?
                    CONSTANTS::SUBSCRIPTION['SLUG']['PREMIUM']
                    : CONSTANTS::SUBSCRIPTION['SLUG']['PLATINUM'];
                update_user_meta(get_current_user_id(), Constants::SUBSCRIPTION['USER_META_KEY'], ['timestamp' => time(), 'PLAN' => $plan]);
            }
        }
    }
